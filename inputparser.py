...
"""
This module developed on the basis of 
...
def recursive_class_creation(section, level, class_dictionary, version_dictionary):
    ...
    imports = ["from xcp2k.inputsection import InputSection"]
    docstring = ""
    properties = ""
    setters = ""
    public = (
    ...
    for subsection in section.findall("SECTION"):
        ...
        imports.append("from xcp2k.classes.{0} import {0}".format(member_class_name))
...
def main(xml_path):
    ...
    for class_name, class_body in class_dictionary.items():
        with open('xcp2k/classes/{}.py'.format(class_name), 'w') as file:
            print(class_body[0])
            file.write(class_body[0] + "\n\n\n")
            class_body_header = (
                "class " + class_name + "(InputSection):\n"
            )
            # print(class_body_header)
